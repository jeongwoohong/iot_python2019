...
#학습 전용, 테스트 전용 데이터 분리
train_features, test_features, train_labels, test_lables = train_test_split(features, labels)
...
print('-'*50)
print('학습 데이터 정확도\n:  %.4f' % clf.score(train_features, train_labels))
print('테스터 데이터 정확도\n: %.4f' %clf.score(test_features, test_lables))
...
weights_and_words.sort(key=lambda  x: x[], reverse=True)
