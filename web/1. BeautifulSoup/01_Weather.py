...
for tr in table.find_all('tr'):
    ...
    for td in tds:
            if td.find('a'):
                ...
                humidity = data_correction(tds[8].text)
                wd_direction = data_correction(tds[9].text)
                wd_speed = data_correction(tds[10].text)
                data.append([point, cloud,visibility, temperature,wd_temp, humidity,wd_direction,wd_speed])
print(data)